...
#define NETHER_FILL_STRENGTH 0.25
#define NETHER_FILL_ROLLOFF  0.45
...
vec3 applyNetherAmbientFill(vec3 color, vec3 fillColor) {
    ...
    vec3 desatFill = mix(vec3(dot(fillColor, vec3(0.299, 0.587, 0.114))), fillColor, 0.7);
    return color + desatFill * NETHER_FILL_STRENGTH * fillMask;
}
